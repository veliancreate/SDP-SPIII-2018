 ...
 */
fun splitInHalf(s: String): Pair<String, String> = Pair(s.substring(0, s.length / 2), s.substring(s.length / 2, s.length))
 ...
 */
fun sumChars(cc: Array<Char>): Int {
    var total = 0
    for (c in cc) {
        total += c.toInt()
    }
